...
void skimTreeMulti(string fileList, string DetNums, Int_t gencut=0, int beamGen=1, int test=0){
  ...
  for(Int_t k=0; k<(Int_t)(detectorNumbers.size()); k++){
    if(k==0 && detectorNumbers.size()==1){
      cout << "{" << detectorNumbers[k] << "}";
    }else if(k==0){
      cout << "{" << detectorNumbers[k];
    }else if(k==((Int_t)(detectorNumbers.size())-1)){
  ...
  for(Int_t i = 0; i < (Int_t)detectorNumbers.size(); i++){
  ...
  for(Int_t d=0; d<(Int_t)(detectorNumbers.size()); d++){
    outputFiles[d]->cd();
    outputTrees[d]->Write();
    outputFiles[d]->Close();
    Int_t gTotal(0);
...
long processOne(string fnm){
  ...
  for (long i=0; i < nEntries;i++){
    ...
    for(int j=0; j<(Int_t)hit->size(); j++){
      if((*hit)[j].mtrid <= generation && abs((*hit)[j].pid)==11 && (*hit)[j].det!=28){
        cout << "Found track!" << endl << "  Entry: " << i << ", Hit.Det: " <<  (*hit)[j].det << endl;
        ...
        if( iterTrid == hitTracks.end() ){
          ...
          if( iterDet != detectorNumbers.end() ){
            ...
            for(Int_t k=0; k<(Int_t)(hitTracks.size()); k++){
              if(k==0 && hitTracks.size()==1){
                cout << "{" << hitTracks[k] << "}" << endl;
              }else if(k==0 ){
                cout << "{" << hitTracks[k];
              }else if(k==((Int_t)(hitTracks.size())-1)){
                cout << "," << hitTracks[k] << "}" << endl;
              }else{
                cout << "," << hitTracks[k];
              }
