...
// Fixed size dimensions of array or collections stored in the TTree if any.
 ...
 //vector<remollGenericDetectorSumByPID_t> sum_by_pid[kMaxsum];
   ...
   ferrous_analysis(TTree *tree=0);
   virtual ~ferrous_analysis();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop(Int_t par, Int_t prim);
...
ferrous_analysis::ferrous_analysis(TTree *tree) : fChain(0)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
/*   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("o_ferrous_extgen_9010_V2parallel_100k.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("o_ferrous_extgen_9010_V2parallel_100k.root");
      }
      f->GetObject("T",tree);
