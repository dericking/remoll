//void skimTreeDet28(string finNm, int testRun=0, int beamGen=0)
...
// >.L analysis/secondarygen/skimTreeDet28.C
// > skimTreeDet28("/volatile/halla/moller12gev/palatchi/remoll_ferrous_parallel_100M_20211211/remoll_ferrous_parallel_100M_20211211_100kEv_001/remollout.root",0,1)
//.q
//
// OR point to a textfile list of root files with file names  
// > skimTreeDet28("mylist.txt",0,1)
// > .q
//
...
long processOne(string fnm){
  ...
  for (long i=0; i < nEntries;i++){
    ...
    for(int j=0; j<hit->size(); j++){
      if(hit->at(j).det == mydet && hit->at(j).pid == 11)
	{//make cut to just collect electrons
