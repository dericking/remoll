...
Bool_t ferrous_analysis::IsInBoundingBox(const Double_t* p) {
    ...
    if (p[0] < -1.*fMax[kR] || p[0] > fMax[kR]){ return false; }
    if (p[1] < -1.*fMax[kR] || p[1] > fMax[kR]){ return false; }
    return true;
}
...
{
    // Check the bounding box; if outside field maps the values which Bfield points to remain unaltered
