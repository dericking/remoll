...
CONFIG="remoll-ferrous-2022-11-07-fasteners";
NUMSIMS=1000;
DETNUMA=(9010 9020 9030 9928) #TODO: WILL EVENTUALLY WANT TO READ FROM FERROUS DET.LIST
...
for THISSIM in $(seq ${SEQSTRT} ${SEQENDS}); do
  ...
  echo "/remoll/step/killLateGen"                                       >> ${FILE};
...
mv primsim_* ${SLRMOUTPT};

mv remollout_*.root ${PRIROOTDR};
mv remoll_*.mac    ${PRIMACDIR};
...
############################################################################################
FILE="slurm_secondary_0.sh";
touch ${FILE}
echo "#!/bin/bash"                                   >> ${FILE};
echo "#SBATCH --partition=production"                >> ${FILE};
echo "#SBATCH --job-name=${CONFIG}"                  >> ${FILE};
echo "#SBATCH --ntasks=1"                            >> ${FILE};
echo "#SBATCH --output=${SOUTDIR}/secsim_0_%a.out"     >> ${FILE};
echo "#SBATCH --error=${SOUTDIR}/secsim_0_%a.err"      >> ${FILE};
...
echo "cd ${SOUTDIR}"                                 >> ${FILE};
echo " "                                             >> ${FILE};
echo "./remoll ferrous_\${SLURM_ARRAY_TASK_ID}_V2parallel_0.mac" >> ${FILE};
cp ${FILE} ${SRCDIR}/jobs/${CONFIG}/ ;

FILE="slurm_secondary_1.sh";
touch ${FILE}
echo "#!/bin/bash"                                   >> ${FILE};
echo "#SBATCH --partition=production"                >> ${FILE};
echo "#SBATCH --job-name=${CONFIG}"                  >> ${FILE};
echo "#SBATCH --ntasks=1"                            >> ${FILE};
echo "#SBATCH --output=${SOUTDIR}/secsim_1_%a.out"     >> ${FILE};
echo "#SBATCH --error=${SOUTDIR}/secsim_1_%a.err"      >> ${FILE};
echo "#SBATCH --array=${DETLIST}"                    >> ${FILE};
echo "#SBATCH --account=halla"                       >> ${FILE};
echo "#SBATCH --mem-per-cpu=1000"                    >> ${FILE};
echo " "                                             >> ${FILE};
echo "cd ${SOUTDIR}"                                 >> ${FILE};
echo " "                                             >> ${FILE};
echo "./remoll ferrous_\${SLURM_ARRAY_TASK_ID}_V2parallel_1.mac" >> ${FILE};
cp ${FILE} ${SRCDIR}/jobs/${CONFIG}/ ;

#This will execute slurm batch array for secondary simulations
sbatch --wait slurm_secondary_0.sh;
wait;
sbatch --wait slurm_secondary_1.sh;
wait;
...
mv ferrous_*.mac   ${SECMACDIR};
mv o_ferrous*.root ${SECROOTDR};
mv slurm_*.sh      ${SLRMSHELL};
