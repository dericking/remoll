...
CONFIG="2022-04-14_remoll-ferrous-sm-test-nobatch";
NUMSIMS=20;
...
tar czf default.tar.gz -C ${SRCDIR}/build remoll libremoll.so reroot\
    ...
    -C ${SRCDIR} ./geometry \
    -C ${SRCDIR}/analysis/ferrous skimTreeMulti.C ferrous_analysis.C ferrous_analysis.h;
...
echo "#SBATCH --array=1-${#DETNUMA[@]}"              >> ${FILE};
echo "#SBATCH --account=halla"                       >> ${FILE};
echo "#SBATCH --mem-per-cpu=5000"                    >> ${FILE};
echo " "                                             >> ${FILE};
echo "cd ${SOUTDIR}"                                 >> ${FILE};
...
echo "  ./remoll ferrous_\${DETS[\$I]}_V2.mac"       >> ${FILE};
echo "done;"                                         >> ${FILE};
...
############################################################################################
for I in ${!DETNUMA[@]}; do
  FILE="ferrous_${DETNUMA[$I]}_V2.mac";
...
./reroot -b -q skimTreeMulti.C+'("rootfiles.list","'${DETLIST}'",1,0)';
...
#mv remoll_*.root   primaryRootFiles;
